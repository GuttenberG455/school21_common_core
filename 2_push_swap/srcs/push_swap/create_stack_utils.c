...
/*   Updated: 2021/12/08 15:02:23 by majacqua         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	unsigned int	mid_order;

	env->min = get_value_by_order(env->stack_a, 0);
	env->max = get_value_by_order(env->stack_a,
			ft_lstack_len(env->stack_a) - 1);
	mid_order = (ft_lstack_len(env->stack_a) - 1) / 2 + 1;
	env->mid = get_value_by_order(env->stack_a, mid_order);
	return (0);
}
