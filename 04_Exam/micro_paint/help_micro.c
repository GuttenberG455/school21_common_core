...
int free_all(FILE *file, char *draw) // очистка памяти 
{
    fclose(file);
    if (draw)
        free(draw);
...
int check_zone(t_zone *zone) // проверка ввведенного размера поля
{
...
char *get_zone(FILE *file, t_zone *zone) // создаем поле
{
    ...
    if ((count = fscanf(file, "%d %d %c\n", &zone->width, &zone->height, &zone->backgrond)) != 3) // берем из файла 1 строку
        return (NULL);
    if (count == (-1))
        return (NULL);
    if (!(check_zone(zone))) // неправильные размеры поля
        return (NULL);
