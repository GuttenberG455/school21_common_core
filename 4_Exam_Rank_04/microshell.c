...
int size_argv(char **argv) // считаем длину аргументов
{
    int i = 0;
...
int parser_argv(t_base **ptr, char **argv) // парсер команды
{
    ...
    new->type = check_end(argv[new->size]);	// записываем тип конца (ничего, пайп, брейк)
...
void exec_cmd(t_base *temp, char **env) // выполнение команды
{
	...
	int pipe_open;	// флаг открытия пайпа

	pipe_open = 0;	// обнуляем флаг
	...
	{
		if (temp->type == TYPE_PIPE && dup2(temp->fd[STDOUT], STDOUT) < 0) // если тип - пайп, и подменяем текущий fd out на STDOUT
			exit_fatal(); // если не получилось, то ошибка
		if (temp->prev && temp->prev->type == TYPE_PIPE && dup2(temp->prev->fd[STDIN], STDIN) < 0) // если предыдущий - пайп, то подменяем предыдущий fd in на STDIN
			exit_fatal(); // если не получилось, то ошибка
	...
	else // родительский процесс
	{
		...
		{
			close(temp->fd[STDOUT]);	// закрыть текущий fd out
			if (!temp->next || temp->type == TYPE_BREAK) // если ничего нет дальше, или текущий тип брейк
				close(temp->fd[STDIN]); // закрываем текущий fd in
		}
		if (temp->prev && temp->prev->type == TYPE_PIPE) // если предыдущий тип - пайп
			close(temp->prev->fd[STDIN]); //  закрываем предыдущий fd in
	}
}
...
0.4 Четыре функции под ошибки: общая, две на cd, и execve
...
2.2 Проходим все структуры по списку
	...
	2.2.2 Если не cd то создаем функцию выполнения команды
	2.2.3.0 Проверяем, если текущий или предыдущий тип команды является пайпом, делаем флаг пайпа на 1, создаем пару фд
	2.2.3.1 Создаем дочерний процесс. Доч. процесс:
		Если тип - |, то заменяем фд out на STDOUT.
		Если предыдущий тип - |, то заменяем предыдыщий фд in на STDIN
		Выполняем команду с аргументами
		Выходим с процесса с успехом, если что-то не получилось из выше стоящего то с ошибкой.
	2.2.3.1 Родительский процесс:
		Ждем дочерний процесс.
		Если флаг пайпа = 1, то закрываем fd out.
			Если нет след или текущий тип ; то закрываем и тек. fd in
		Если предыдущий тип |, то закрываем предыдущий fd in
3.0 Освобождаем всю выделенную память	
